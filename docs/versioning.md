...
The EUD Remote Assist Portal uses a single release version in the repo root **`VERSION`** file (currently **2.0.0**). Install automation and operators can read it from git or from the running API.

## Source of truth

| Location | Format |
|----------|--------|
| `VERSION` (repo root) | `2.0.0` (semver, one line) |
| Git tag (optional) | `v2.0.0` |
...
```bash
git tag -a v2.0.0 -m "EUD Remote Assist Portal 2.0.0"
